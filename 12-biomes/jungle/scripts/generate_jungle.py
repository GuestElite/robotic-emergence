...
def render_unit_air_jungle():
    """Chasseur delta enemy en thème jungle (camouflage vert + glow vert acide).
    ...
    # Ombre projetée au sol (suggère l'altitude)
    ...
    # Réacteurs arrière (glow vert acide à la sortie)
...
def render_factory_air_jungle():
    """Hangar à toit ouvert enemy en thème jungle. Sprite dessiné en orientation
    'player' (toit en haut) puis flippé verticalement → toit ouvre vers le sud."""
    W = H = 128
    pal = ENEMY_JUNGLE
    img, d = new_canvas(W, H)

    # Plateforme (symétrique)
    d.rounded_rectangle((6, 8, W - 6, H - 6), radius=10,
    ...
    # Lumières de balisage (glow vert acide jungle au lieu du jaune désert)
    for cx, cy in [(20, 90), (W - 20, 90), (34, 16), (W - 34, 16)]:
        sd.ellipse((cx - 2, cy - 2, cx + 2, cy + 2),
                   fill=pal["glow"], outline=ACCENT["outline"])

    # Porte arrière de service (en bas dans le sprite, deviendra en haut après flip)
    gate_w = 22
    ...
    # Flip vertical → toit ouvre vers le sud (vers la base joueur)
...
def main():
    ...
    to_render = [
        ...
        ("unit-air-enemy.png",         render_unit_air_jungle),
